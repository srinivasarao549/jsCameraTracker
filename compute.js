function compute(currentFrame, lastFrame, width, height, threshold, matrix, dd){
    ...
    switch (matrix) {
        ...
        case "kirsch":
            mXSize = 3;
            mYSize = 3;
            mX = [[-5, 3, 3], [-5, 0, 3], [-5, 3, 3]];
            mY = [[-5, -5, -5], [3, 0, 3], [3, 3, 3]];
            break;
        case "harris":
            mXSize = 5;
            mYSize = 1;
    ...
    for (var y = 0; y < height; y++) {
        for (var x = 0; x < width; x++) {
            // dX
            npix = 0;
            for (var pX = -1 * ((mXSize - 1) / 2); pX <= ((mXSize - 1) / 2); pX++) {
            
                for (var pY = -1 * ((mYSize - 1) / 2); pY <= ((mYSize - 1) / 2); pY++) {
                
                    npix += v(x + pX, y + pY) *
                    mX[pY + (mYSize - 1) / 2][pX + (mXSize - 1) / 2];
            ...
            for (var pX = -1 * ((mXSize - 1) / 2); pX <= ((mXSize - 1) / 2); pX++) {
                ...
                for (var pY = -1 * ((mYSize - 1) / 2); pY <= ((mYSize - 1) / 2); pY++) {
                    ...
                    npix += v(x + pX, y + pY) *
                    mY[pY + (mYSize - 1) / 2][pX + (mXSize - 1) / 2];
...
onmessage = function(event){
    ...
    function getGreyFrame(frame){
        var i = 0;
        greyTab = [];
        for (var y = 0; y < frame.length; y += 1) {
            var line = frame[y].split(";");
            for (var x = 0; x < line.length; x += 1) {
                var tmp = parseInt(line[x]);
                greyTab[i] = (((tmp >> 16) & 0xff) + ((tmp >> 8) & 0xff) + tmp & 0xff) / 6;
                i++;
            }
        }
        return greyTab;
    ...
    var res = compute(getGreyFrame(data.currentFrame), getGreyFrame(data.lastFrame), data.width, data.height, data.threshold, data.matrix, data.displayDiff);
    postMessage({
        "key": "result",
        "val": res
    });
